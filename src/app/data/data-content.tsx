    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {error && (
        <div className={`mb-6 rounded-lg p-4 flex items-center gap-3 ${
          ...
          <div className={`w-5 h-5 flex-shrink-0 ${error.includes('✅') ? 'text-green-600' : 'text-blue-600'}`}>
            {error.includes('✅') ? '✓' : '🔍'}
          </div>
          <p className={`flex-1 font-medium ${error.includes('✅') ? 'text-green-800' : 'text-blue-800'}`}>
            {error}
          </p>
        </div>
      )}
